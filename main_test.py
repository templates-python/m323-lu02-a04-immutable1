...
def test_students_is_list():
    ...
    assert isinstance(main.students, list), f'Expected a list, but got {type(main.students)}.'
...
def test_students_tuple_structure():
    ...
    for student in main.students:
        ...
        assert isinstance(name, str), f'Expected a string for name, but got {type(name)}.'
        assert isinstance(age, int), f'Expected an int for age, but got {type(age)}.'
        assert isinstance(student_id, int), f'Expected an int for student_id, but got {type(student_id)}.'
